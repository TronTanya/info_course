...
export function MentorMessageMeta({ meta }: { meta?: TutorPipelineMeta }) {
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-1.5 border-t border-cyan/10 pt-2">
      <Badge variant={levelVariant[level]} className="font-mono text-[10px] uppercase tracking-wide">
      ...
      {hint ? <span className="w-full text-[10px] text-muted-foreground">{hint}</span> : null}
    </div>
  );
}

...
export function LearningSidebarPanel({
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      <div>
        <p className="typo-eyebrow text-primary">Модули курса</p>
        <ul className="mt-3 max-h-[min(40vh,280px)] space-y-1 overflow-y-auto pr-1">
          {modules.map((m) => (
            <li key={m.id}>
              ...
              >
                <span className="font-mono text-[10px] tabular-nums opacity-70">{m.orderNumber}</span>
                ...
                {clickable && step.actionHref ? (
                  <Link
                    ...
                    className={cn(
                      ...
                      step.isActive && "bg-primary/12 font-medium text-primary ring-1 ring-primary/30",
                      !step.isActive && "text-foreground hover:bg-muted/50",
                    )}
        ...
        {open ? (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 left-0 z-50 flex w-[min(100%,20rem)] flex-col border-r border-border bg-card shadow-2xl lg:hidden"

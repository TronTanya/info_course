      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(220px,280px)] lg:items-start">
        <div className="min-w-0 space-y-4">
          ...
          <dl className="grid grid-cols-3 gap-2 sm:max-w-md">
            <MetricPill iconKind="lesson" label="Уроки" text={formatLessonCount(contentCounts.lessons)} />
            <MetricPill iconKind="test" label="Тесты" text={formatTestCount(contentCounts.tests)} />
            <MetricPill iconKind="practice" label="Практика" text={formatPracticeCount(contentCounts.practices)} />
          </dl>
          {progressRow ? (
...
function MetricPill({
  ...
  return (
    <div className="flex flex-col items-center gap-1 rounded-xl border border-border/70 bg-muted/20 px-2 py-2 text-center">
      <CourseStepIcon kind={iconKind} size="sm" status="available" />

...
function filterWhere(filter: string | undefined): Prisma.SubmissionWhereInput {
  ...
  if (f === "submitted") return { status: { in: ["SUBMITTED", "CHECKING"] } };
  if (f === "retry") return { status: { in: ["NEEDS_REVISION", "REJECTED"] } };
...
const TABS: { href: string; label: string; match: string }[] = [
  ...
  { href: "/admin/submissions?filter=pending", label: "На проверке", match: "pending" },
  { href: "/admin/submissions?filter=submitted", label: "Отправлено", match: "submitted" },
  { href: "/admin/submissions?filter=retry", label: "Нужна доработка", match: "retry" },
...
type Props = { searchParams: Promise<{ filter?: string; student?: string }> };

export default async function AdminSubmissionsPage({ searchParams }: Props) {
  await ensureAdminPageAccess();
  const { filter, student: studentUserId } = await searchParams;
  const active = filter ?? "all";

  const rows = await prisma.submission.findMany({
    where: {
      ...filterWhere(filter),
      ...(studentUserId?.trim() ? { userId: studentUserId.trim() } : {}),
    },
    ...
    <AdminShell>
      <div className="space-y-6">
        <AdminPageHeader
          ...
          description={
            studentUserId?.trim()
              ? "Отправки выбранного студента. После «Принято» пересчитывается прогресс модуля."
              : "Отправки со статусом не «черновик». После «Принято» пересчитывается прогресс модуля и курс."
          }
        />
